...
func (m *Application) Validate() error {
	...
	for idx, item := range m.GetSchedulerCluster() {
		_, _ = idx, item

		if v, ok := interface{}(item).(interface{ Validate() error }); ok {
			if err := v.Validate(); err != nil {
				return ApplicationValidationError{
					field:  fmt.Sprintf("SchedulerCluster[%v]", idx),
					reason: "embedded message failed validation",
					cause:  err,
				}
			}
	...
	for idx, item := range m.GetCdnClusters() {
		_, _ = idx, item

		if v, ok := interface{}(item).(interface{ Validate() error }); ok {
			if err := v.Validate(); err != nil {
				return ApplicationValidationError{
					field:  fmt.Sprintf("CdnClusters[%v]", idx),
					reason: "embedded message failed validation",
					cause:  err,
				}
			}
