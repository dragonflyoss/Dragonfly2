...
func (d *d7yBalancerBuilder) Build(cc balancer.ClientConn, opts balancer.BuildOptions) balancer.Balancer {
	b := &d7yBalancer{
		cc:             cc,
...
func (d *d7yBalancerBuilder) Name() string {
...
func (c *d7yBalancer) UpdateClientConnState(s balancer.ClientConnState) error {
	c.resolverErr = nil
...
func (c *d7yBalancer) ResolverError(err error) {
	c.resolverErr = err
	if len(c.subConns) == 0 {
...
func (c *d7yBalancer) regeneratePicker() {
	if c.state == connectivity.TransientFailure {
	...
	c.picker = NewD7yReportingPicker(readySCs, c.pickResultChan)
}

// mergeErrors is copied from baseBalancer.
func (c *d7yBalancer) mergeErrors() error {
	if c.connErr == nil {
...
func (c *d7yBalancer) UpdateSubConnState(sc balancer.SubConn, state balancer.SubConnState) {
...
func (c *d7yBalancer) Close() {}

// resetSubConn will replace a SubConn with a new idle SubConn.
func (c *d7yBalancer) resetSubConn(sc balancer.SubConn) error {
...
func (c *d7yBalancer) getSubConnAddr(sc balancer.SubConn) (string, error) {
	v, ok := c.scInfos.Load(sc)
	if !ok {
...
func (c *d7yBalancer) resetSubConnWithAddr(addr string) error {
...
func (c *d7yBalancer) scManager() {
	...
	go func() {
		for {
			...
			c.pickResults.Add(pickResult{Ctx: shadowCtx, SC: pr.SC})
			c.scCountsLock.Lock()
	...
	go func() {
		for {
			v, shutdown := c.pickResults.Get()
			if shutdown {
				time.Sleep(connectionLifetime)
				continue
			}
			pr := v.(pickResult)
			c.pickResults.Done(v)
			select {
			case <-pr.Ctx.Done():
				c.scCountsLock.Lock()
				cnt, ok := c.scCounts[pr.SC]
