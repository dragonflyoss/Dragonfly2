...
func main() {
	...
	config := &transport.ClientTlsConfigBuilder{
	...
	client := transport.NewTCPClient(
		...
		transport.WithClientSslEnabled(true),
		transport.WithClientTlsConfigBuilder(config),
...
func NewHelloClientSession(session transport.Session) (err error) {
	tls.EventListener.SessionOnOpen = func(session transport.Session) {
