...
func getPieceMetaValue(record *cdn.PieceMetaRecord) string {
	return fmt.Sprintf("%d:%d:%s:%s:%d:%d", record.PieceNum, record.PieceLen, record.Md5, record.Range, record.Offset, record.PieceStyle)
}

func parsePieceMetaRecord(value string) (record *cdn.PieceMetaRecord, err error) {
	defer func() {
