...
func newShareMemManager(taskMgr mgr.SeedTaskMgr, memoryStore store.StorageDriver) *ShareMemManager {
	return &ShareMemManager{
		switcher:     newShmSwitcherService(),
		spaceGcTimer: newShmSpaceGc(),
		taskMgr: taskMgr,
...
func (sm *ShareMemManager) tryShmSpace(ctx context.Context, url, taskId string, fileLength int64) (string, error) {
	if sm.switcher.check(url, fileLength) && sm.spaceGcTimer.canUseShm() {
		...
		usableSpace, err := sm.getUsableSpace(ctx)
		if err != nil {
		...
		if useShm { // 创建shm
			...
			return sm.memoryStore.GetPath(raw), nil
		}
 ...
 * @return
 */
func (sm *ShareMemManager) getUsableSpace(ctx context.Context) (fileutils.Fsize, error) {
	totalSize, freeSize, err :=sm.memoryStore.GetTotalAndFreeSpace(ctx, &store.Raw{})
	if err != nil {
		return 0, err
	}
