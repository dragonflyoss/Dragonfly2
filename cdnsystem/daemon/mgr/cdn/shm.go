...
type ShareMemManager struct {
	switcher     shmSwitcherService
	spaceGcTimer shmSpaceGcTimer
}

func (sm *ShareMemManager) tryShmSpace(url, taskId string, fileLength int64) (string, error){
...
func (sm *ShareMemManager) getUsableSpace() (fileutils.Fsize, error) {
	totalSize, freeSize, err := fileutils.GetTotalAndFreeSpace(config.ShmHome)
	if err != nil {
		return 0, err
	}
