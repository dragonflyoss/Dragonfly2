...
func (s *SuperWriterTestSuite) SetUpSuite(c *check.C) {
	...
	fileStore, err := store.NewStore(config.LocalStorageDriver, store.NewLocalStorage, s.config)
...
func (s *SuperWriterTestSuite) TestStartWriter(c *check.C) {
	...
	checkFileSize(s.writer.cdnStore, task.TaskID, expectedSize, c)
}
...
func (s *SuperWriterTestSuite) TestWriteToFile(c *check.C) {
	...
	err := s.writer.writeToFile(context.TODO(), bb, task.TaskID, 0, pieceContSize, task.PieceSize, nil)
	c.Check(err, check.IsNil)

	checkFileSize(s.writer.cdnStore, task.TaskID, int64(pieceSize), c)
}
