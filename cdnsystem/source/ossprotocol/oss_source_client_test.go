...
func (s *OssSourceClientTestSuite) TestParseOssObject() {
	...
	tests := []struct {
	...
	}{
		{
			...
			name: "test2",
			args: args{
				ossUrl: "oss://xx/rowkey_20191010144421mYtlKyATuW_app.txt",
			},
			want: &ossObject{
				bucket: "xx",
			...
			name: "test3",
			args: args{
				ossUrl: "http://xxx/aaa/rowkey_20191010144421mYtlKyATuW_app.txt",
			},
			wantErr: true,
		},
	}
...
func (s *OssSourceClientTestSuite) TestGetContentLength() {
	...
	tests := []struct {
	...
	}{
		{
			name: "test1",
			args: args{
				url: "oss://xxx/rowkey_20191010144421mYtlKyATuW_app.txt",
				header: map[string]string{
					"endpoint":        "http://oss-xxx.com",
				},
			},
