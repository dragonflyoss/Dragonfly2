...
var (
	defaultPeerHostConfigPath = "/etc/dragonfly/peerhost.yml"
)
...
func initDaemonFlags() {
	...
	err := flagDaemonOpt.Load(defaultPeerHostConfigPath)
	if err != nil {
		panic(err)
	}
	...
	flagSet.DurationVar(&flagDaemonOpt.Storage.Option.TaskExpireTime, "expire-time", flagDaemonOpt.Storage.Option.TaskExpireTime, "caching duration for which cached file keeps no accessed by any process, after this period cache file will be deleted")
	...
	flagSet.IPVar(&flagDaemonOpt.Host.AdvertiseIP, "advertise-ip", flagDaemonOpt.Host.AdvertiseIP, "the ip report to scheduler, normal same with listen ip")
