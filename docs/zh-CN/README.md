...
Dragonfly Document 是关于 Dragonfly 的介绍。对于一般对 Dragonfly 感兴趣的人，这个 repo 的 README.md 就足够了。对于最终用户来说，`/docs` 中包含的所有详细信息是最好的指南。而对于开发者来说，Developer Guide 部分的内容才是最需要的。
...
在此页面中找到 `WIP` 或 `TODO` 并关注 [CONTRIBUTING](../../CONTRIBUTING.md)。

## 快速开始

[快速入门](user-guide/quick-start.md) 如果您想尝试 Dragonfly，这正是您所需要的。本文件包括什么
先决条件，
如何安装 Dragonfly 以及如何体验 Dragonfly 的使用。

## 用户指南

[用户指南](user-guide/README.md) 帮助终端用户体验 Dragonfly 所需的各种指南。不仅非常简短 [快速入门]
(user-guide/quick-start.md)，但是详细的二进制安装和配置说明。此外，任何有助于用户的概念和功能
更好地理解蜻蜓也将包括在内。
...
命令行是体验 Dragonfly 能力的最简单方法。 API 扩展将为 Dragonfly 带来更多更进一步的体验。 Commandline只是API的一种组合用法，如果你想破解或利用Dragonfly的更多优势，请参见[API参考](./api-reference)。与命令行文档一样，所有 API 文档都是通过源代码自动生成的。

## 生态

生态系统文档显示了 Dragonfly 与云原生生态系统中流行的工具或系统之间的联系。他们指导最终用户如何体验云
...
[开发指南](development/local.md) 帮助（潜在的）开发者/贡献者理解 Dragonfly 内部的理论而不是它公开的接口。和
...
文件夹`/docs` 不包含有关蜻蜓的所有文档。在此 repo 的其他路径中还有其他非常有用的文档，例如：
...
如果您正在搜索某个文档但没有找到，请不要犹豫 [提交问题](https://github.com/dragonflyoss/Dragonfly2/issues/new/choose) 寻求帮助。另外，如果您发现文档中有一些不正确的地方或错别字，请帮助提交拉取请求以更正。