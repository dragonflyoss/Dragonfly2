...
for c in ${components}; do
  ...
  sed -i "s#RUN make build-$c && make install-$c#RUN --mount=type=cache,target=/root/.cache/go-build --mount=type=cache,target=/go/pkg export GOPATH=/root/go \&\& make build-$c \&\& make install-$c#" "${file}"
  echo $c Dockerfile:
  cat $file
done
...
sed -i "s#RUN make build-dfget && make install-dfget#RUN --mount=type=cache,target=/root/.cache/go-build --mount=type=cache,target=/go/pkg export GOPATH=/root/go \&\& make build-dfget \&\& make install-dfget#" build/images/dfdaemon/Dockerfile
echo dfdaemon Dockerfile:
cat build/images/dfdaemon/Dockerfile
