...
sed -i "s#RUN make build-dfget && make install-dfget#RUN --mount=type=cache,target=/root/.cache/go-build --mount=type=cache,target=/go/pkg make build-dfget \&\& make install-dfget#" build/images/dfdaemon/Dockerfile
