...
func (s *Scheduler) ExchangeParents(peer *supervisor.Peer, blankParents sets.String) (*supervisor.Peer, []*supervisor.Peer) {
	// target can exchange
	...
	for {
		...
		peer.Log().Debugf("for parent %s, exchange in %s and exchange out %s", parent.PeerID, into.PeerID, out.PeerID)

		out.ReplaceParent(nil)
		into.ReplaceParent(parent)
		into = out
		candidateParents = s.selectCandidateParents(into, s.cfg.CandidateParentCount, blankParents)
...
func (s *Scheduler) ScheduleParent(peer *supervisor.Peer, blankParents sets.String) (*supervisor.Peer, []*supervisor.Peer, bool) {
	...
	if len(candidateParents) == 0 {
		peer, candidateParents = s.ExchangeParents(peer, blankParents)
		if len(candidateParents) == 0 {
			return nil, nil, false
		}
	}
