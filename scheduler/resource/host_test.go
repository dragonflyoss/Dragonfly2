	...
	tests := []struct {
	...
	}{
		{
			...
			expect: func(t *testing.T, host *Host) {
				...
				assert.Equal(host.OS, mockRawHost.OS)
				assert.Equal(host.Platform, mockRawHost.Platform)
				assert.Equal(host.PlatformFamily, mockRawHost.PlatformFamily)
				assert.Equal(host.PlatformVersion, mockRawHost.PlatformVersion)
				assert.Equal(host.KernelVersion, mockRawHost.KernelVersion)
		...
		},
		{
			...
			expect: func(t *testing.T, host *Host) {
				...
				assert.Equal(host.OS, mockRawSeedHost.OS)
				assert.Equal(host.Platform, mockRawSeedHost.Platform)
				assert.Equal(host.PlatformFamily, mockRawSeedHost.PlatformFamily)
				assert.Equal(host.PlatformVersion, mockRawSeedHost.PlatformVersion)
				assert.Equal(host.KernelVersion, mockRawSeedHost.KernelVersion)
				assert.Equal(host.ConcurrentUploadLimit.Load(), int32(config.DefaultPeerConcurrentUploadLimit))
				assert.Equal(host.ConcurrentUploadCount.Load(), int32(0))
				assert.Equal(host.UploadCount.Load(), int64(0))
				assert.Equal(host.UploadFailedCount.Load(), int64(0))
