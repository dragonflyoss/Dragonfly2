...
func Test_NewNetworkTopology(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			n, err := NewNetworkTopology(tc.config, rdb, res, mockStorage)
			tc.expect(t, n, err)
		})
	}
}
...
func TestNetworkTopology_LoadDestHostIDs(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			mock: func(clientMock redismock.ClientMock) {
				clientMock.ExpectKeys(pkgredis.MakeNetworkTopologyKeyInScheduler(mockSeedHost.ID, "*")).SetErr(errors.New("destination hosts do not exist"))
			},
			expect: func(t *testing.T, n NetworkTopology, err error) {
				assert := assert.New(t)
				assert.Nil(err)
	...
	tests := []struct {
	...
	}{
		{
			...
			mock: func(clientMock redismock.ClientMock) {
				...
				clientMock.ExpectDel(pkgredis.MakeProbesKeyInScheduler("*", mockSeedHost.ID)).SetVal(1)
				clientMock.ExpectDecrBy(pkgredis.MakeProbedCountKeyInScheduler(mockSeedHost.ID), 1).SetVal(1)
			},
			expect: func(t *testing.T, n NetworkTopology, err error) {
				assert := assert.New(t)
				assert.Nil(err)
		...
		},
		{
			...
			mock: func(clientMock redismock.ClientMock) {
				clientMock.ExpectDel(pkgredis.MakeNetworkTopologyKeyInScheduler(mockSeedHost.ID, "*")).SetErr(errors.New("delete network topology error"))
			},
			expect: func(t *testing.T, n NetworkTopology, err error) {
				assert := assert.New(t)
				assert.Nil(err)
		...
		},
		{
			...
			mock: func(clientMock redismock.ClientMock) {
				clientMock.MatchExpectationsInOrder(true)
				clientMock.ExpectDel(pkgredis.MakeNetworkTopologyKeyInScheduler(mockSeedHost.ID, "*")).SetVal(1)
				clientMock.ExpectDel(pkgredis.MakeProbesKeyInScheduler(mockSeedHost.ID, "*")).SetErr(errors.New("delete probes which sent by host error"))
			},
			expect: func(t *testing.T, n NetworkTopology, err error) {
				assert := assert.New(t)
				assert.Nil(err)
		...
		},
		{
			...
			mock: func(clientMock redismock.ClientMock) {
				...
				clientMock.ExpectDel(pkgredis.MakeNetworkTopologyKeyInScheduler(mockSeedHost.ID, "*")).SetVal(1)
				clientMock.ExpectDel(pkgredis.MakeProbesKeyInScheduler(mockSeedHost.ID, "*")).SetVal(1)
				clientMock.ExpectDel(pkgredis.MakeProbesKeyInScheduler("*", mockSeedHost.ID)).SetErr(errors.New("delete probes which sent to host error"))
			},
			expect: func(t *testing.T, n NetworkTopology, err error) {
				assert := assert.New(t)
				assert.Nil(err)
...
func TestNetworkTopology_ProbedCount(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			mock: func(clientMock redismock.ClientMock) {
				clientMock.ExpectGet(pkgredis.MakeProbedCountKeyInScheduler(mockHost.ID)).SetVal("1")
			},
			expect: func(t *testing.T, n NetworkTopology, err error) {
				assert := assert.New(t)
				assert.Nil(err)
		...
		},
		{
			...
			mock: func(clientMock redismock.ClientMock) {
				clientMock.ExpectGet(pkgredis.MakeProbedCountKeyInScheduler(mockHost.ID)).SetErr(errors.New("host do not exist"))
			},
			expect: func(t *testing.T, n NetworkTopology, err error) {
				assert := assert.New(t)
				assert.Nil(err)
