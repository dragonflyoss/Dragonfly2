...
func (m *manager) ListPeersByTask(taskID string) []*supervise.Peer {
	var peers []*supervise.Peer
...
func NewManager(cfg *config.GCConfig, hostManager supervise.HostMgr) supervise.PeerMgr {
	m := &manager{
...
var _ supervise.PeerMgr = (*manager)(nil)

func (m *manager) Add(peer *supervise.Peer) {
	m.lock.Lock()
	defer m.lock.Unlock()
...
func (m *manager) Get(peerID string) (*supervise.Peer, bool) {
...
func (m *manager) Pick(task *supervise.Task, limit int, pickFn func(peer *supervise.Peer) bool) (pickedPeers []*supervise.Peer) {
	return m.pick(task, limit, false, pickFn)
}

func (m *manager) PickReverse(task *supervise.Task, limit int, pickFn func(peer *supervise.Peer) bool) (pickedPeers []*supervise.Peer) {
	return m.pick(task, limit, true, pickFn)
}

func (m *manager) pick(task *supervise.Task, limit int, reverse bool, pickFn func(peer *supervise.Peer) bool) (pickedPeers []*supervise.Peer) {
	if pickFn == nil {
		return
	}
	if !reverse {
	...
	for range m.cleanupExpiredPeerTicker.C {
		m.peerMap.Range(func(key, value interface{}) bool {
			...
			if elapse > m.peerTTI && !peer.IsDone() {
				...
				peer.SetStatus(supervise.PeerStatusZombie)
			}
