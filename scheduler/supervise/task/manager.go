...
func NewManager(cfg *config.GCConfig, peerManager supervise.PeerMgr) supervise.TaskMgr {
	m := &manager{
...
var _ supervise.TaskMgr = (*manager)(nil)

func (m *manager) Delete(taskID string) {
	m.taskMap.Delete(taskID)
}

func (m *manager) Add(task *supervise.Task) {
	m.taskMap.Store(task.TaskID, task)
}

func (m *manager) Get(taskID string) (task *supervise.Task, ok bool) {
	item, ok := m.taskMap.Load(taskID)
	if !ok {
		return nil, false
	}
	return item.(*supervise.Task), true
}

func (m *manager) GetOrAdd(task *supervise.Task) (actual *supervise.Task, loaded bool) {
